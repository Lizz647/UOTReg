...
file = "dynamics/statefate/traj_bundle.h5"
...
# traj_reg: array [G x N x T] = [1000 x 2381 x 9]
# Step 0: quick sanity check
stopifnot(length(dim(traj_reg)) == 3)
G <- dim(traj_reg)[1]; N <- dim(traj_reg)[2]; Tm <- dim(traj_reg)[3]

## ## ## ## ## ## ## ## ## ## ## ## ## 
# Starting with within-cluster analysis
## ## ## ## ## ## ## ## ## ## ## ## ## 
...
time_vec_orig <- seq(2, 6, by = 1)  # 1.5, 4.5, ..., 25.5
N <- dim(traj_reg)[2]
Tm <- dim(traj_reg)[3]
...
# ---- Example: SOX2 ----
plot_gene_quick("Mmp8", n_per_cluster = 10, facet_scales = "fixed")
...
#   )
#   
#   result_folder = "/Users/binghao/Desktop/Research/OT-singlecell/UOTReg/results/dynamics/statefate/LMM"
#   jsonlite::write_json(
#     payload,
