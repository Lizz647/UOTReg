## ## ## ## ## ## ## ## ## ## ## ## ## 
## Direct composing: 5 time steps ## 
## ## ## ## ## ## ## ## ## ## ## ## ## 

library(rhdf5)
result_folder <- "/Users/binghao/Desktop/Research/OT-singlecell/UOTReg/results/"
file = "dynamics/embryoid/Final_result/traj_bundle_dir.h5"
...
stopifnot(Tm == 5, length(lab_reg_km) == N)

time_vec_orig <- seq(1.5, 25.5, by = 6)            # 1.5, 4.5, ..., 25.5
t_centered    <- time_vec_orig - mean(time_vec_orig)

cell_ids <- paste0("cell", seq_len(N))

# ---- Choose gene (fuzzy match) ----
gene_pat <- "GATA6"  # <-- change here
...
model_map["5"] <- "2p"   # cluster 4 -> three-piece
model_map["1"] <- "2p"   # cluster 2 -> two-piece
...
#   )
#   
#   result_folder = "/Users/binghao/Desktop/Research/OT-singlecell/UOTReg/results/dynamics/embryoid/LMM/dir"
#   jsonlite::write_json(
#     payload,
...
#   outfile    = paste(genenow,"_lmm_viz_dir.json",sep="")
# )

## ## ## ## ## ## ## ## ## ##
## Mioflow: 9 time steps ## 
## ## ## ## ## ## ## ## ## ## 

library(rhdf5)
result_folder <- "/Users/binghao/Desktop/Research/OT-singlecell/UOTReg/results/"
file = "dynamics/embryoid/traj_bundle_mio.h5"
file_path = paste(result_folder, file, sep = "")
...
length(genes_idx)        # how many genes remain
dim(traj_reg)        # should be [length(genes_idx) x 2381 x 9]
...
cell_ids <- if (exists("cell_ids")) as.character(cell_ids) else paste0("cell", seq_len(N))
...
#   )
#   
#   result_folder = "/Users/binghao/Desktop/Research/OT-singlecell/UOTReg/results/dynamics/embryoid/LMM/mio"
#   jsonlite::write_json(
#     payload,
#     path        = file.path(result_folder, outfile),
#     dataframe   = "rows",
...
#   break_lines_all = break_lines_all,
#   model_map  = model_map,           # if you have it; otherwise pass setNames(rep("1p", 6), 0:5)
#   outfile    = paste(genenow,"_lmm_viz_mio.json",sep="")
# )
