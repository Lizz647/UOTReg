...
def matrix_sqrt_via_diagonalization(A):
    # Eigen-decomposition
    ...
    # Construct diagonal matrix of square root eigenvalues
    sqrt_D = np.diag(sqrt_eigenvalues)
    
    # Reconstruct the square root of the original matrix
    A_sqrt = eigenvectors @ sqrt_D @ np.linalg.inv(eigenvectors)
    
    return A_sqrt

# # Example matrix
# A = np.array([[14, 2], [89, 33]], dtype=float)

# # Compute the square root of the matrix
# A_sqrt = matrix_sqrt_via_diagonalization(A)
# print("Square root of the matrix A:\n", A_sqrt)
# print("Check (A_sqrt^2):\n", A_sqrt @ A_sqrt)
...
class DatasetSampler(Sampler):
    def __init__(self, dataset, batch_size, num_workers=4, device='cuda'):
...
class NormalSampler(Sampler):
    def __init__(
        ...
        if weight is None:
            ...
            # weight = symmetrize(matrix_sqrt_via_diagonalization(self.cov))
